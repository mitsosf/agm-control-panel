...
@section('content')
    <div class="container">
        <h5 style="text-align: center; color: green;">{{$returnedCount.'/'.$devicesCount}} ({{$ratio}}%)</h5>
            ...
            <table id="example2" class="table table-bordered table-hover">
                ...
                @foreach($delegations as $delegation)
                    @if(!is_null($delegation))
                        @if($delegation->given == 1)
                            <tr>
                                <td><a class="btn btn-success" href="{{route('voting.return.validate',$delegation->id)}}">Take
                                        device</a></td>
